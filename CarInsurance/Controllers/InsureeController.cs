...
namespace CarInsurance.Controllers
{
    public class InsureeController : Controller
    {
        ...
        {           
            ...
            if (insuree.CarMake == "Porshe") quote += 25;
            if (insuree.CarMake == "Porsche" && insuree.CarModel == "911 Carrera") quote += 25;

            //Adjust quote based on number of speeding tickets
            if (int.TryParse(insuree.SpeedingTickets, out int numSpeedingTickets)) quote += numSpeedingTickets * 10;

            //Adjust quote if user has had a DUI
            if (insuree.DUI) quote += quote * 0.25F;

            //Adjust quote if they want full insurance
            if (insuree.CoverageType) quote += quote * 0.50F;

            insuree.Quote =(int) quote;            
        }
